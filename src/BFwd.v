...
Description:
    ...
    - This is not exactly like the AFwd because some instructions use rs2 and some use immediates
...
module BFwd (
    ...
    assign sameX = inst_FD[24:20] == inst_X[11:7];
    assign sameM = inst_FD[24:20] == inst_M[11:7];
    assign sameW = inst_FD[24:20] == inst_W[11:7];

    wire noWriteRS2;
    
    assign noWriteRS2 = op_FD != `OPC_ARI_ITYPE & op_FD != `OPC_LOAD & op_FD != `OPC_JAL & op_FD != `OPC_JALR & op_FD != `OPC_AUIPC & op_FD != `OPC_LUI & op_FD != `OPC_CSR;

    always @(*) begin
        // outer case statement checks for opcodes in the M stage that write.
        if (inst_FD[24:20] != `x0 & noWriteRS2) begin
